...
class Gym2OpEnv(gym.Env):
    ...
    def setup_observations(self):
        ...
        self._gym_env.observation_space = self._gym_env.observation_space.keep_only_attr(["rho", "gen_p", "load_p", "topo_vect", 
                                                                      "actual_dispatch"])
    ...
    def setup_actions(self):
        ...
        dim_act_space = np.sum([np.sum(self._gym_env.action_space[el].shape) for el in self._gym_env.action_space.spaces])
        ...
        new_dim_act_space = np.sum([np.sum(self._gym_env.action_space[el].shape) for el in self._gym_env.action_space.spaces])
        print(f"The new size of the action space is : {new_dim_act_space}")
        
        self._gym_env.action_space = self._gym_env.action_space.reencode_space("redispatch", ContinuousToDiscreteConverter(nb_bins=8))
